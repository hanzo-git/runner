### Running `git-runner` using `docker-compose`

```yml
...
  git-server:
    image: ghcr.io/hanzoai/git-server
    ...
    healthcheck:
      # checks availability of Git's front-end with curl
    ...
    environment:
      ...
      # The Git server version must be v1.23 or higher.
  ...
  runner:
    image: hanzo-git/runner
    restart: always
    depends_on:
      git-server:
        # required so runner can attach to the git-server, see "healthcheck"
...
### Running `git-runner` using Docker-in-Docker (DIND)
...
...
  runner:
    image: hanzo-git/runner:latest-dind-rootless
    restart: always
    privileged: true
    security_opt:
