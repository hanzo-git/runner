...
DOCKER_IMAGE ?= ghcr.io/hanzoai/git-runner
DOCKER_TAG ?= nightly
...
TAGS ?=
LDFLAGS ?= -X "github.com/hanzo-git/runner/internal/pkg/ver.version=v$(RELASE_VERSION)"
...
go-check:
	...
	@if [ "$(GO_VERSION)" -lt "$(MIN_GO_VERSION)" ]; then \
		echo "Git Runner requires Go $(MIN_GO_VERSION_STR) or greater to build. You can get it at https://go.dev/dl/"; \
		exit 1; \
	fi
