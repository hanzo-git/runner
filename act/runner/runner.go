...
type Config struct {
	...
	// self-hosted Git (DEFAULT_ACTIONS_URL=self). It gates token trust: only then may the
	...
	InsecureSkipTLS                   bool                         // whether to skip verifying TLS certificate of the Git instance
	MaxParallel                       int                          // max parallel jobs to run across all workflows (0 = no limit, uses CPU count)
	AllocatePTY                       bool                         // allocate a pseudo-TTY for each step's process
}

// GetToken: Adapt to Git
func (c Config) GetToken() string {
	token := c.Secrets["GITHUB_TOKEN"]
	if c.Secrets["GIT_TOKEN"] != "" {
		token = c.Secrets["GIT_TOKEN"]
	}
	return token
}
...
type caller struct {
	...
	updateResultLock         sync.Mutex        // For Git
