...
func (rc *RunContext) jobContainerName() string {
	...
	return createContainerName(nameParts...) // For Git
}

// networkNameForHost return the name of the network
func (rc *RunContext) networkNameForHost() (string, bool) {
...
func (rc *RunContext) startJobContainer() common.Executor {
	return func(ctx context.Context) error {
		...
		name := rc.jobContainerName()
		// For Git, to support --volumes-from <container_name_or_id> in options.
		...
		networkName, createAndDeleteNetwork := rc.networkNameForHost()
		...
		// For Git, `jobContainerNetwork` should be the same as `networkName`
...
func (rc *RunContext) Executor() (common.Executor, error) {
	...
	return func(ctx context.Context) error {
		...
		if err != nil {
			rc.caller.setReusedWorkflowJobResult(rc.JobName, "failure") // For Git
			return err
		}
		if res {
			return executor(ctx)
...
func (rc *RunContext) getGithubContext(ctx context.Context) *model.GithubContext {
	...
	{ // Adapt to Git
		if preset := rc.Config.PresetGitHubContext; preset != nil {
			...
			ghc.APIURL = instance + "/api/v1" // the version of Git is v1
			ghc.GraphQLURL = ""               // Git doesn't support graphql
			return ghc
		}
	}
...
func (rc *RunContext) withGithubEnv(ctx context.Context, github *model.GithubContext, env map[string]string) map[string]string { //nolint:unparam // pre-existing issue from nektos/act
	...
	{ // Adapt to Git
		...
		env["GITHUB_API_URL"] = instance + "/api/v1" // the version of Git is v1
		env["GITHUB_GRAPHQL_URL"] = ""               // Git doesn't support graphql
