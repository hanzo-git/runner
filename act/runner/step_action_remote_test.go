...
func TestStepActionRemoteUsesGitHubInstanceWhenDefaultActionInstanceEmpty(t *testing.T) {
	...
	assert.Equal(t, "https://hanzo.example/actions/setup-go", actualURL)
	sarm.AssertExpectations(t)
}
...
func Test_newRemoteAction(t *testing.T) {
	tests := []struct {
		...
		{
			action: "https://git.hanzo.ai/actions/heroku@main", // it's invalid for GitHub, but self-hosted Git instances support it
			want: &remoteAction{
			...
			wantCloneURL: "https://git.hanzo.ai/actions/heroku",
		},
		{
			action: "https://git.hanzo.ai/actions/aws/ec2@main", // it's invalid for GitHub, but self-hosted Git instances support it
			want: &remoteAction{
				URL:  "https://git.hanzo.ai",
				Org:  "actions",
				Repo: "aws",
			...
			wantCloneURL: "https://git.hanzo.ai/actions/aws",
		},
		{
			action: "http://git.hanzo.ai/actions/heroku@main", // it's invalid for GitHub, but self-hosted Git instances support it
			want: &remoteAction{
				URL:  "http://git.hanzo.ai",
				Org:  "actions",
				Repo: "heroku",
				Path: "",
				Ref:  "main",
			},
			wantCloneURL: "http://git.hanzo.ai/actions/heroku",
		},
		{
			action: "http://git.hanzo.ai/actions/aws/ec2@main", // it's invalid for GitHub, but self-hosted Git instances support it
			want: &remoteAction{
				URL:  "http://git.hanzo.ai",
				Org:  "actions",
				Repo: "aws",
			...
			wantCloneURL: "http://git.hanzo.ai/actions/aws",
		},
		{
			action: "ssh://git@git.hanzo.ai/actions/heroku@main", // it's invalid for GitHub, but self-hosted Git instances support it
			want: &remoteAction{
				URL:  "ssh://git@git.hanzo.ai",
				Org:  "actions",
				Repo: "heroku",
				Path: "",
				Ref:  "main",
			},
			wantCloneURL: "ssh://git@git.hanzo.ai/actions/heroku",
		},
		{
			action: "ssh://git@git.hanzo.ai/actions/aws/ec2@main", // the ssh user is kept as part of the host segment
			want: &remoteAction{
				URL:  "ssh://git@git.hanzo.ai",
				Org:  "actions",
				Repo: "aws",
			...
			wantCloneURL: "ssh://git@git.hanzo.ai/actions/aws",
		},
		{
			action: "ssh://git.hanzo.ai/onlyonesegment@main", // missing org/repo after the host
			want:   nil,
		},
	}
...
func TestStepActionRemoteCloneTokenSurvivesNilSecrets(t *testing.T) {
	...
	table := []struct {
	...
	}{
		{
			...
			gitHubInstance: "hanzo.example.com",
