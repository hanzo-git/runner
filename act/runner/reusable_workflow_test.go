...
// Regression test for hanzo-git/runner#37483: a remote reusable workflow at a moving
...
func TestReusableWorkflowCachedBranchRefRefreshes(t *testing.T) {
	...
	require.NoError(t, os.MkdirAll(filepath.Join(workDir, ".github/workflows"), 0o755))
...
func TestGetGitCloneTokenWithSchemalessGitInstance(t *testing.T) {
	conf := &Config{
		GitHubInstance: "hanzo.example.net",
		Secrets: map[string]string{
			"GIT_TOKEN": "token-value",
		},
	}

	token := getGitCloneToken(conf, "https://hanzo.example.net/actions/tools")
...
func TestGetGitCloneTokenSelfHostedActionsDifferentHost(t *testing.T) {
	...
	conf := &Config{
		GitHubInstance:                    "hanzo.local",
		DefaultActionInstance:             "https://hanzo.my-nas.lan",
	...
	token := getGitCloneToken(conf, "https://hanzo.my-nas.lan/owner/action")
...
func TestShouldCloneURLUseToken(t *testing.T) {
	tests := []struct {
	...
	}{
		{
			...
			instanceURL: "hanzo.example.net",
			cloneURL:    "https://hanzo.example.net/actions/tools",
			want:        true,
		},
		{
			name:        "same host with schemaless instance and port",
			instanceURL: "hanzo.example.net:3000",
			cloneURL:    "https://hanzo.example.net:3000/actions/tools",
			want:        true,
		},
		{
			name:        "different host",
		...
		},
		{
			...
			instanceURL: "hanzo.example.net",
			cloneURL:    "https://user:pass@hanzo.example.net/actions/tools",
			want:        false,
		},
		{
			name:        "invalid clone URL",
			instanceURL: "hanzo.example.net",
			cloneURL:    "://hanzo.example.net/actions/tools",
			want:        false,
		},
		{
			...
			instanceURL:           "hanzo.local",
			trustedActionInstance: "https://hanzo.my-nas.lan",
			cloneURL:              "https://hanzo.my-nas.lan/owner/action",
			want:                  true,
		},
		{
			...
			instanceURL:           "hanzo.local",
			trustedActionInstance: "https://hanzo.my-nas.lan",
			cloneURL:              "https://user:pass@hanzo.my-nas.lan/owner/action",
			want:                  false,
		},
		{
