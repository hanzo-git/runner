...
//  1. Git doesn't support specifying GithubContext.ServerURL by the GITHUB_SERVER_URL env
//  2. Git has already full URL with rc.Config.GitHubInstance when calling newRemoteReusableWorkflowWithPlat
//
...
func newReusableWorkflowExecutor(rc *RunContext, directory, workflow string) common.Executor {
	return func(ctx context.Context) error {
		...
		// return runner.NewPlanExecutor(plan)(ctx)
		return common.NewPipelineExecutor( // For Git
...
func NewReusableWorkflowRunner(rc *RunContext) (Runner, error) {
	runner := &runnerImpl{
		...
		caller: &caller{
			...
			reusedWorkflowJobResults: map[string]string{}, // For Git
		},
	}
...
func (r *remoteReusableWorkflow) CloneURL() string {
	// In Git, r.URL always has the protocol prefix, we don't need to add extra prefix in this case.
...
// workflows from `.github/workflows` and `.github/workflows` are supported
...
// For Git
// getGitCloneToken returns GIT_TOKEN when shouldCloneURLUseToken returns true,
...
//  1. cloneURL's host matches this Git instance: either the registered instance
