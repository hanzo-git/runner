...
func (sar *stepActionRemote) prepareActionExecutor() common.Executor {
	return func(ctx context.Context) error {
		...
		gitClone := stepActionRemoteNewCloneExecutor(git.NewGitCloneExecutorInput{
			...
			InsecureSkipTLS: sar.cloneSkipTLS(), // For Git
		})
...
// For Git
// cloneSkipTLS returns true if the runner can clone an action from the Git instance
func (sar *stepActionRemote) cloneSkipTLS() bool {
	if !sar.RunContext.Config.InsecureSkipTLS {
		// Return false if the Git instance is not an insecure instance
		return false
	}
	if sar.remoteAction.URL == "" {
		// Empty URL means the default action instance should be used
		// Return true if the URL of the Git instance is the same as the URL of the default action instance
		return sar.RunContext.Config.DefaultActionURL() == sar.RunContext.Config.GitHubInstance
	}
	// Return true if the URL of the remote action is the same as the URL of the Git instance
