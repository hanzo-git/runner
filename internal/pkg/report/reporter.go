...
func NewReporter(ctx context.Context, cancel context.CancelFunc, client client.Client, task *runnerv1.Task, cfg *config.Config) *Reporter {
	...
	if v := task.Context.Fields["git_runtime_token"].GetStringValue(); v != "" {
		oldnew = runner.AppendSecretMasker(oldnew, v)
	}
...
func (r *Reporter) runDaemonLoop() {
	...
	for {
		...
		case <-r.ctx.Done():
			// Stop heartbeating on cancel so Git sees the runner as offline
...
func (r *Reporter) Close(lastWords string) error {
	...
	// Git's UpdateLog short-circuits on len(Rows)==0 before honoring NoMore,
