...
import (
	...
	"github.com/hanzo-git/runner/act/artifacts"
...
func runExec(ctx context.Context, execArgs *executeArgs) func(cmd *cobra.Command, args []string) error {
	return func(cmd *cobra.Command, args []string) error {
		...
		if len(execArgs.artifactServerPath) == 0 {
			tempDir, err := os.MkdirTemp("", "git-act-")
			if err != nil {
				fmt.Println(err)
			}
		...
		if t := config.Secrets["GIT_TOKEN"]; t != "" {
			config.Token = t
...
func loadExecCmd(ctx context.Context) *cobra.Command {
	...
	execCmd.PersistentFlags().StringVarP(&execArg.workflowsPath, "workflows", "W", "./.github/workflows/", "path to workflow file(s)")
	...
	execCmd.PersistentFlags().StringVarP(&execArg.image, "image", "i", "ghcr.io/hanzoai/runner-ubuntu:latest", "Docker image to use. Use \"-self-hosted\" to run directly on the host.")
	execCmd.PersistentFlags().StringVarP(&execArg.network, "network", "", "", "Specify the network to which the container will connect")
	execCmd.PersistentFlags().StringVarP(&execArg.githubInstance, "git-instance", "", "", "Git instance to use.")
