...
type Poller struct {
	...
	runner       TaskRunner
	cfg          *config.Config
	tasksVersion atomic.Int64 // tasksVersion used to store the version of the last task fetched from the Git.
...
func (p *Poller) Shutdown(ctx context.Context) error {
	...
	case <-ctx.Done():
		...
		// wait for running jobs to report their status to Git
		<-p.done
